...
if (room = rCultistBattle){
	...
	if (menuState == "MAIN"){
		...
		if (optionState = "ATTACK") || (magicState == "CHOOSE TARGET") || (optionState == "CHOOSE TARGET"){
			if (optionTarget.sprite_index == sCultistLeader){
				draw_sprite_ext(sSelector, 0, optionTarget.x + sprite_get_width(sCultistLeader) / 2, optionTarget.y - 16, 1, 1, 270, c_white, 1);
			}else {
				if (optionTarget.sprite_index == sPlayerBattle){
					draw_sprite_ext(sSelector, 0, optionTarget.x + sprite_get_width(sCultistLeader) / 2, optionTarget.y - sprite_get_height(sSelector), 1, 1, 270, c_white, 1);
				}
			}
		}
		if (optionState == "MAGIC"){
